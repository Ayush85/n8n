...
const WEB_PUSH_SUBJECT =
  process.env.WEB_PUSH_SUBJECT || "mailto:support@aydexis.com";
...
if (WEB_PUSH_ENABLED) {
  ...
  logger.warn(
    "Native web push is disabled. Set WEB_PUSH_PUBLIC_KEY and WEB_PUSH_PRIVATE_KEY to enable it.",
  );
}
...
app.get("/api/push/public-key", (req, res) => {
  if (!WEB_PUSH_ENABLED) {
    return res
      .status(503)
      .json({ error: "Web push is not configured on server" });
  }
...
app.post("/api/push/unsubscribe", async (req, res, next) => {
  try {
    ...
    await pool.query(
      "DELETE FROM push_subscriptions WHERE endpoint = $1 AND role = $2",
...
app.get("/api/push/subscriptions/stats", async (req, res, next) => {
  try {
    ...
    const [adminResult, userResult] = await Promise.all([
      pool.query(
        "SELECT COUNT(*)::int AS count FROM push_subscriptions WHERE role = 'admin'",
      ),
      pool.query(
        "SELECT COUNT(*)::int AS count FROM push_subscriptions WHERE role = 'user'",
      ),
    ]);
...
app.post("/api/push/test-admin", async (req, res, next) => {
  try {
    ...
    const {
      title = "Test Push",
      body = "Native push test from API",
      url,
    } = req.body || {};
...
app.post("/api/upload", upload.single("file"), async (req, res, next) => {
  try {
    ...
    io.emit("admin_alert", {
      ...
      userName:
        fileSessionRow?.customer_name ||
        fileSessionRow?.user_contact ||
        "Customer",
...
const N8N_WEBHOOK_TIMEOUT_MS = parseInt(
  process.env.N8N_WEBHOOK_TIMEOUT_MS || "60000",
  10,
);
...
async function sendAdminPushNotification({ heading, content, url }) {
  ...
  if (webPushResult.sent > 0) {
    logger.info(
      `Native web push sent to ${webPushResult.sent} admin subscriber(s)`,
    );
    return;
  }

  logger.warn(
    "No active admin web-push subscription found for this notification.",
  );
}
...
function parseN8nResponse(data) {
  ...
  if (parsed && typeof parsed === "object") {
    const message =
      parsed.output || parsed.text || parsed.message || parsed.result;
    if (message) {
  ...
  try {
    ...
    try {
      if (data?.answer) {
        ...
        // fallback: n8n already returns clean JSON
        ai = data;
      }
    } catch (err) {
    ...
    // � DB save (AWAITED - ensure persistence before responding)
    try {
      await pool.query(
        "INSERT INTO messages(session_id, sender, content) VALUES($1,$2,$3)",
        [sessionId, "ai", output],
      ...
      // Continue anyway - don't fail the entire response
    }

    // 🚀 socket emit (safe) - emit AFTER DB is confirmed
    if (io && sessionId) {
      io.to(sessionId).emit("new_message", {
        sender: "ai",
        content: output,
...
app.post("/webhook/products", async (req, res) => {
  try {
    ...
    try {
      ...
      logger.info(
        `Product webhook processed: ${inserted} inserted, ${updated} updated`,
      );
...
app.post("/api/messages", async (req, res) => {
  try {
    ...
    const { sessionId, sender, content, metadata = {} } = data.data;
    ...
    await Promise.allSettled([
      pool.query(
        `
                ...
                DO UPDATE SET
                    updated_at = NOW(),
                    metadata = COALESCE(sessions.metadata, '{}'::jsonb) || $2::jsonb
                `,
    ...
    // ----------------------------------
    // background admin logic (non-blocking safe)
    // ----------------------------------
    if (sender === "user") {
      (async () => {
        try {
          ...
          const userName =
            row?.customer_name || row?.user_contact || "Customer";

          const isHumanSession = row?.status === "human";
          ...
          if (isHumanSession) {
            await sendAdminPushNotification({
              heading: `Message from ${userName}`,
              content: String(content).slice(0, 140),
              url: getPublicAppUrl(),
            });
          }
        } catch (err) {
          console.error("Admin background error:", err);
        }
      })();
    }
  } catch (error) {
...
app.get("/api/sessions/by-contact/:contact", async (req, res, next) => {
  ...
  try {
    ...
    logger.info(
      `Found ${result.rows.length} session(s) for contact ${contact}`,
    );
  ...
  try {
    ...
    res.json({
      ...
      avgMessagesPerSession: parseFloat(
        avgMessagesPerSession.rows[0]?.avg || 0,
      ),
      peakHours: peakHours.rows,
      avgResponseTimeSeconds: parseFloat(
        avgResponseTime.rows[0]?.avg_seconds || 0,
      ),
    });
  } catch (error) {
    next(error);
    ...
    for (const session of sessionsResult.rows) {
      ...
      if (messagesResult.rows.length > 0) {
        ...
        sessionSummaries.push(
          `--- Session: ${session.customer_name || session.session_id} (${session.status}) ---\n${convo}`,
        );
      }
    }
...
Return ONLY valid JSON, no other text.`,
    ...
    try {
      ...
      logger.error(
        "Failed to parse GPT report response:",
        gptResponse.choices[0].message.content,
      );
      report = {
  ...
  socket.on("send_manual_message", async (data) => {
    try {
      ...
      if (userContact) {
        ...
        logger.warn(
          `Skipping user push for ${sessionId}; no user_contact available.`,
        );
      }
