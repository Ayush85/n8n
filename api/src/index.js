  ...
  try {
    ...
    const response = await fetch(N8N_WEBHOOK_URL, {
      ...
      body: JSON.stringify({
        ...
        chatInput,
        metadata,
        session_id: sessionId, // for backward compatibility with older n8n workflows
        query: chatInput, // for backward compatibility with older n8n workflows
      }),
    });
    ...
    console.log("Raw response from n8n webhook:", text);
    ...
    if (assembled) {
      ai = JSON.parse(assembled);
    } else {
      // Response is plain JSON, not a stream
      ai = JSON.parse(text);
    }
    ...
    return res.status(500).json({
      error: `Failed to process chat response: ${err.message}`,
    });
  }
});
